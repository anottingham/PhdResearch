...
protocol returns [Protocol value]
		:	'protocol' i = ID {$value = new Protocol($i.text);} '{' (f = field[$value]  {$value.AddField($f.value);})+ (s = switch[$value] {$value.Switch = $s.value;})? '}' 
        ;
...
sum_op	returns [MathOperator value]
		:	'+'		{$value = MathOperator.Add; }
		|	'-'		{$value = MathOperator.Subtract; }
		;

mult_op returns [MathOperator value]
		:	'*'		{$value = MathOperator.Multiply; }
