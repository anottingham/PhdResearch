...
#define DISTILL_READ_WIDTH (DISTILL_READ_WIDTH_KB * 1024)
#define DISTILL_BLOCK_SIZE (DISTILL_BLOCK_SIZE_MB * 1024 * 1024)
#define DISTILL_READS_PER_BLOCK (DISTILL_BLOCK_SIZE / DISTILL_READ_WIDTH)
...
void DistillReader(void* distillOptions)
{
	...
	writer_socket.bind("inproc://distill_raw");
	...
	FILE * file;
	errno_t error = fopen_s(&file, options->capture_name, "rb"); //open file
	if (error != 0) { POST_ERROR("Distiller Reader Thread", "File could not be opened."); }	
	
	char* header;
	empty_socket.recv(&header, sizeof(char*));
	fread(header, sizeof(char), 24, file);

	writer_socket.send(&header, sizeof(char*));
	...
	for (int k = 0; k < options->task_count; k++)
	{
		...
		empty_socket.recv(&data, sizeof(char*));
		...
		writer_socket.send(&task, sizeof(DistillTask));
	}
...
void DistillWriter(void* distillOptions)
{
	...
	PcapWriter writer(options, header);

	used_socket.send(&header, sizeof(char*));

	int discarded = 0;
	for (int k = 0; k < options->task_count; k++)
	{
		DistillTask task;
		reader_socket.recv(&task, sizeof(DistillTask));
		
		writer.LoadRange(task.index_start, task.index_count);
...
void Distiller(zmq::context_t& context, zmq::socket_t& client_socket)
{
	...
	DistillerOptions * options = new DistillerOptions();
